...
Normalizes with background intensity.
'''
import os
...
def load_and_preprocess_image(filename, norm):
    # Load the image
    ...
    return image*norm*10

images = [load_and_preprocess_image(os.path.join(img_path, file), norm) for file, norm in tqdm.tqdm(zip(filenames, normalization))]
