...
Uses background intensity to normalize.
'''
import numpy as np
import os
import pandas as pd
...
for image in tqdm.tqdm(movie.frames(), total=n_frames):
    ...
    if len(data_frame)>0:
        snippets = extract_cells(image, data_frame, image_size, crop=True, background=background)
        predictions = model.predict(np.array([snippet*norm*10 for snippet, norm in snippets]))
